...
_MODEL_SERVICE_CLASS = ModelService
modelService = ModelService()


def _model_service():
    if ModelService is not _MODEL_SERVICE_CLASS:
        return ModelService()
    return modelService
...
def model_list():
    try:
        return R.success(_model_service().get_all_models(True),msg="获取模型列表成功")
...
def delete_model(model_id: int):
    try:
        success = _model_service().delete_model_by_id(model_id)
        if success:
...
def model_list(provider_id):
    ...
    return R.success(_model_service().get_all_models_by_id(provider_id))
...
def get_enabled_models_by_provider(provider_id: str):
    try:
        models = _model_service().get_enabled_models_by_provider(provider_id)
        return R.success(models, msg="获取启用模型成功")
    except Exception as e:
        logger.exception("获取启用模型失败")
